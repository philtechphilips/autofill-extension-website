      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        >
          <div className="inline-flex items-center gap-2 bg-white/[0.03] border border-white/[0.05] rounded-full px-4 py-1.5 mb-8">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-white/40">Security Architecture</span>
          </div>

          <h2 className="text-4xl md:text-5xl font-bold text-white tracking-tight mb-6">
            Privacy as a first principle.
          </h2>
          <p className="text-lg text-white/40 max-w-5xl mx-auto font-light">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {securityFeatures.map((feature, index) => (
            ...
            >
              <div className="w-10 h-10 rounded-lg bg-white/[0.03] border border-white/[0.05] flex items-center justify-center text-white/30 group-hover:text-white transition-colors duration-500 mb-6 font-bold uppercase tracking-widest text-xs">
                {feature.icon}
              </div>
              <h3 className="text-sm font-semibold text-white tracking-tight mb-3">{feature.title}</h3>
              <p className="text-[13px] text-white/30 leading-relaxed font-light">{feature.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
