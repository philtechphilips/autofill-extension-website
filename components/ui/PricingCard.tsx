    ...
    >
      ...
      <div className="flex flex-col gap-10">
        <div className="space-y-4">
          <h3 className="text-sm font-semibold text-white/50 uppercase tracking-widest">{name}</h3>
          <div className="flex items-baseline gap-1">
            <span className="text-4xl font-bold text-white tracking-tighter">{price}</span>
            {period && <span className="text-white/30 text-xs">/{period}</span>}
          </div>
        </div>

        <ul className="flex flex-col gap-4">
          {features.map((feature, index) => (
            <li key={index} className="flex items-start gap-4 text-[13px] text-white/40 font-light">
